...
#(
    ...
    parameter   MAC_BOARD   =   48'h12_34_56_78_9A_BC   ,
    //板卡IP地址
    parameter   IP_BOARD    =   {8'd10,8'd19,8'd37,8'd123}
)
(
...
always @(posedge sys_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        ...
        && (des_ip[23:0] == IP_BOARD[31:8]) && (data == IP_BOARD[7:0]))
        sw_en   <=  1'b1;
...
//mac_flag:MAC地址正确标志
assign  mac_flag = ((state == ETH_HEAD) && (des_mac == MAC_BOARD))
                    ? 1'b1 : 1'b0;
...
//ip_flag:IP地址正确标志
assign  ip_flag = ((des_ip[23:0] == IP_BOARD[31:8])
            && (data == IP_BOARD[7:0])) ? 1'b1 : 1'b0;
//ip_len:IP首部字节长度
always@(posedge sys_clk or negedge sys_rst_n)
    if(sys_rst_n == 1'b0)
        ip_len  <=  6'b0;
